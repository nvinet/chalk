 ...
 *    cascade, because a mapping is meaningless once its exercise is gone.
 *  - `schema/sessions.ts` — what happened. Cascades from its own session, but
 *    *restricts* against the catalogue: an exercise history mentions cannot be
 ...
 * (exercise, muscle group) pairing, and SQLite does not enforce foreign keys
