 ...
 * none of the historical data exercises a distance machine or a warm-up flag —
 * both postdate it.
 */
...
const rower: Machine = { ...lift, id: "rower", name: "Rower", tracking: "duration" };
const jog: Machine = { ...lift, id: "jog", name: "Jogging", tracking: "distance" };
...
test("a distance machine is logged on distance alone", () => {
  assert.equal(isSetLogged(set({ machineId: "jog", distanceM: 5000 }), "distance"), true);
});

test("a distance machine with no distance is not logged", () => {
  assert.equal(isSetLogged(set({ machineId: "jog", durationSeconds: 1800 }), "distance"), false);
});

test("distance counts a muscle group as met", () => {
  const s = session([set({ machineId: "jog", muscleGroupId: "chest", distanceM: 5000 })]);
  assert.equal(evaluateSession(s, { machines: [jog] }).successful, true);
});
...
// as a weights machine, so cardio produced 0 kg bests and flat charts.

test("a distance machine records no personal best of 0 kg", () => {
  const s = session([set({ machineId: "jog", distanceM: 5000 })]);
  assert.deepEqual(personalBestsInSession(s, [], { machines: [jog] }), []);
});

test("a distance machine has no heaviest weight", () => {
  const s = session([set({ machineId: "jog", distanceM: 5000 })]);
  const best = bestsForPairing([s], { machineId: "jog", muscleGroupId: "chest" }, jog);
  assert.equal(best.heaviestKg, null);
});

test("a distance machine charts nothing rather than a line of zeroes", () => {
  const s = session([set({ machineId: "jog", distanceM: 5000 })]);
  assert.deepEqual(topSetSeries([s], { machineId: "jog", muscleGroupId: "chest" }, jog), []);
});

test("last time on a distance machine reads in km, not kg", () => {
  const s = session([set({ machineId: "jog", distanceM: 5000 })]);
  const last = lastTimeForPairing([s], { machineId: "jog", muscleGroupId: "chest" }, jog);
  assert.equal(last?.summary, "5 km");
});

test("last time on a timed machine still reads in minutes", () => {
  const s = session([set({ machineId: "rower", durationSeconds: 1800 })]);
  const last = lastTimeForPairing([s], { machineId: "rower", muscleGroupId: "chest" }, rower);
  assert.equal(last?.summary, "30 min");
});

test("a distance machine contributes no volume", () => {
  assert.equal(setVolumeKg(set({ machineId: "jog", distanceM: 5000 }), jog), 0);
});
...
test("a bodyweight entered as a real load behaves like any other weight", () => {
  ...
  const best = bestsForPairing([s], { machineId: "bench", muscleGroupId: "chest" }, lift);
...
test("a warm-up alone never completes a muscle group", () => {
  ...
  const outcome = evaluateSession(s, { machines: [lift] });
...
test("a warm-up cannot set a personal best", () => {
  ...
  assert.deepEqual(personalBestsInSession(s, [], { machines: [lift] }), []);
});
...
test("indexById still round-trips machines", () => {
